...
class LipsyncFeature extends AbstractHostFeature.mix(
  ...
  constructor(
    ...
    visemeLayers.forEach(
      ({ name, animation, visemeMap = DefaultVisemeMap }) => {
  ...
  async _onViseme({ mark }) {
    Object.entries(this._visemeLayers).forEach(([layerName, animName]) => {
      ...
      const { isActive, visemeMap } =
        this._managedLayers[layerName].animations[animName];
  ...
  playLipSync(totalDuration) {
    this.enable();
    // Track timers so they can be cleared if lip sync is stopped early
    this._lipSyncTimers = this._lipSyncTimers || [];
    this._lipSyncActive = true;

    const visemes = [
      { value: "p", duration: 100 },
      { value: "t", duration: 200 },
      { value: "a", duration: 150 },
      { value: "o", duration: 200 },
      { value: "i", duration: 100 },
      { value: "e", duration: 120 },
      { value: "s", duration: 140 },
      { value: "k", duration: 110 },
    ];

    const totalVisemeDuration = visemes.reduce(
      (acc, viseme) => acc + viseme.duration,
      0
    );
    const scaleFactor = totalDuration ? totalDuration / totalVisemeDuration : 1;

    const handleViseme = async (index) => {
      if (!this._lipSyncActive) {
        return;
      }

      if (index >= visemes.length) {
        // If totalDuration was provided, stop after one pass. Otherwise loop.
        if (totalDuration) {
          this.disable();
          return;
        }
      ...
      await this._onViseme({ mark: viseme });

      const timer = setTimeout(() => handleViseme(index + 1), viseme.duration * scaleFactor);
      this._lipSyncTimers.push(timer);
    };

    handleViseme(0);
  }

  // Override disable to cancel any pending lip-sync timers so lip-sync stops immediately
  disable(seconds, easingFn) {
    this._lipSyncActive = false;
    if (this._lipSyncTimers && this._lipSyncTimers.length) {
      this._lipSyncTimers.forEach((t) => clearTimeout(t));
      this._lipSyncTimers = [];
    }

    // Call parent implementation to turn off layer weights
    if (super.disable) {
      return super.disable(seconds, easingFn);
    }
  }
  ...
  installApi() {
    ...
    Object.assign(api, {
      playLipSync: this.playLipSync.bind(this),
      /**
       * @memberof LipsyncFeature
       * @instance
       * @method
